...
import { MysqlModuleAsyncOptions, MysqlModuleOptions, MysqlOptionsFactory } from './interfaces';
...
export class MysqlCoreModule implements OnApplicationShutdown {
  ...
  public static forRoot(options: MysqlModuleOptions, connection?: string): DynamicModule {
  ...
  public static forRootAsync(options: MysqlModuleAsyncOptions, connection: string): DynamicModule {
    const connectionProvider: Provider = {
      provide: getConnectionToken(connection),
  ...
  async onApplicationShutdown(): Promise<any> {
    const connection = this.moduleRef.get<any>(getConnectionToken(this.options as MysqlModuleOptions) as Type<any>);
    if (connection) {
      await (connection as mysql.Pool).end();
    }
  }

  public static createAsyncProviders(options: MysqlModuleAsyncOptions): Provider[] {
  ...
  public static createAsyncOptionsProvider(options: MysqlModuleAsyncOptions): Provider {
    ...
    return {
      provide: MYSQL_MODULE_OPTIONS,
      useFactory: async (optionsFactory: MysqlOptionsFactory): Promise<MysqlModuleOptions> => {
  ...
  private static async createConnectionFactory(options: MysqlModuleOptions): Promise<any> {
